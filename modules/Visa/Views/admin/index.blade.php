...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{!empty($recovery) ? __('Recovery') : __("All Visas")}}</h1>
            <div class="title-actions">
                @if(empty($recovery))
                <a href="{{route('visa.admin.create')}}" class="btn btn-primary">{{__("Add new visa")}}</a>
                @endif
            </div>
        </div>
        ...
        <div class="filter-div d-flex justify-content-between ">
            ...
            <div class="col-left dropdown">
                <form method="get" action="{{ !empty($recovery) ? route('visa.admin.recovery') : route('visa.admin.index')}}" class="filter-form filter-form-right d-flex justify-content-end flex-column flex-sm-row" role="search">
                    @if(!empty($rows) and $visa_manage_others)
            ...
            <div class="panel-body">
                <form action="" class="bravo-form-item">
                    ...
                    <table class="table table-hover">
                        ...
                        @if($rows->total() > 0)
                            @foreach($rows as $row)
                                <tr class="{{$row->status}}">
                                    ...
                                    <td>
                                        @if(empty($recovery))
                                            <a href="{{route('visa.admin.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Edit')}}</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
                                <td colspan="7">{{__("No visa found")}}</td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
