...
class Visa extends Bookable
{
    use Notifiable;
    use SoftDeletes;
    use CapturesService;

    protected $table = 'bravo_visas';
    public $type = 'visa';
    ...
    public $set_paid_modal_file                = 'Visa::frontend/booking/set-paid-modal';
    public $email_new_booking_file             = 'Visa::emails.new_booking_detail';
    public $availabilityClass = VisaDate::class;
    protected $translation_class = VisaTranslation::class;
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->visaDateClass = VisaDate::class;
        $this->visaTermClass = VisaTerm::class;
        $this->visaTranslationClass = VisaTranslation::class;
    ...
    {
        $meta['seo_title'] = __("Search for Visas");
    ...
    public function terms(){
        return $this->hasMany($this->visaTermClass, "target_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . config('visa.visa_route_prefix') . "/" . $this->slug;
        if(!empty($param)){
    ...
    {
        return url(route('visa.admin.edit',['id'=>$this->id]));
    }
    ...
    public function addToVisat(Request $request)
    {
        $res = $this->addToVisatValidate($request);
    ...
    public function addToVisatValidate(Request $request)
    {
        $rules = [
            'number' => 'required',
        ...
        if(!$this->isAvailableInRanges($start_date,$end_date,$total_number)){
            return $this->sendError(__("This visa is not available at selected dates"));
        }
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('visa_booking_buyer_fees');
        if(!empty($list_fees)){
    ...
    {
        return setting_item("visa_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("visa_review_approved", 0);
    }

    public function review_after_booking(){
        return setting_item("visa_enable_review_after_booking", 0);
    }

    public function count_remain_review()
    {
        $status_making_completed_booking = [];
        $options = setting_item("visa_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("visa_review_stats", []))) {
    ...
    {
        $visa_id = $this->id;
        $list_score = Cache::rememberForever('review_'.$this->type.'_' . $visa_id, function () use ($visa_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $visa_id)->where('object_model', "visa")->where("status", "approved")->first();
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','author_id'])->where('object_id', $this->id)->where('object_model', 'visa')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('visa_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Visas", ['number' => $number]);
        }
        return __(":number Visa", ['number' => $number]);
    }
    ...
    public function saveCloneByID($clone_id){
        ...
        //Language
        $langs = $this->visaTranslationClass::where("origin_id",$old->id)->get();
        if(!empty($langs)){
    ...
    public function isDepositEnable(){
        return (setting_item('visa_deposit_enable') and setting_item('visa_deposit_amount'));
    }
    public function getDepositAmount(){
        return setting_item('visa_deposit_amount');
    }
    public function getDepositType(){
        return setting_item('visa_deposit_type');
    }
    public function getDepositFomular(){
        return setting_item('visa_deposit_fomular','default');
    }
    ...
    public static function isEnableEnquiry(){
        if(!empty(setting_item('booking_enquiry_for_visa'))){
            return true;
        }
        return false;
    }
    public static function isFormEnquiryAndBook(){
        $check = setting_item('booking_enquiry_for_visa');
        if(!empty($check) and setting_item('booking_enquiry_type_visa') == "booking_and_enquiry" ){
            return true;
        }
        return false;
    }
    public static function getBookingEnquiryType(){
        $check = setting_item('booking_enquiry_for_visa');
        if(!empty($check)){
            if( setting_item('booking_enquiry_type_visa') == "only_enquiry" ) {
                return "enquiry";
            }
        }
        return "book";
    ...
    {
        $query = parent::query()->select("bravo_visas.*");
        $query->where("bravo_visas.status", "publish");
        if (!empty($location_id = $request['location_id'] ?? "" )) {
            ...
            if(!empty($location)){
                $query->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_visas.location_id')
        ...
        if (!empty($price_range = $request['price_range'] ?? "")) {
            ...
            $raw_sql_min_max = "( (IFNULL(bravo_visas.sale_price,0) > 0 and bravo_visas.sale_price >= ? ) OR (IFNULL(bravo_visas.sale_price,0) <= 0 and bravo_visas.price >= ? ) )
                            AND ( (IFNULL(bravo_visas.sale_price,0) > 0 and bravo_visas.sale_price <= ? ) OR (IFNULL(bravo_visas.sale_price,0) <= 0 and bravo_visas.price <= ? ) )";
            $query->WhereRaw($raw_sql_min_max,[$pri_from,$pri_from,$pri_to,$pri_to]);
        }

        if($term_id = $request['term_id'] ?? "")
        {
            $query->join('bravo_visa_term as tt1', function($join) use ($term_id){
                $join->on('tt1.target_id', "bravo_visas.id");
                $join->where('tt1.term_id', $term_id);
            });
        }

        if(!empty($request['attrs'])){
            $this->filterAttrs($query,$request['attrs'],'bravo_visa_term');
        }
        ...
        if(!empty( $service_name = $request['service_name'] ?? "" )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $query->leftJoin('bravo_visa_translations', function ($join) {
                    $join->on('bravo_visas.id', '=', 'bravo_visa_translations.origin_id');
                });
                $query->where('bravo_visa_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $query->where('bravo_visas.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        ...
        {
            $query->where('bravo_visas.is_featured',1);
        }
        if (!empty($request['custom_ids']) and !empty( $ids = array_filter($request['custom_ids']) )) {
            $query->whereIn("bravo_visas.id", $ids);
            $query->orderByRaw('FIELD (' . $query->qualifyColumn("id") . ', ' . implode(', ', $ids) . ') ASC');
        }
        $orderby = $request['orderby'] ?? "";
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_visas.sale_price, 0 ) > 0 THEN bravo_visas.sale_price ELSE bravo_visas.price END AS tmp_min_price";
            ...
            default:
                if(!empty($request['order']) and !empty($request['order_by'])){
                    $query->orderBy("bravo_visas.".$request['order'], $request['order_by']);
                }else{
    ...
    public function dataForApi($forSingle = false){
        ...
        if($forSingle){
            ...
            $data['booking_fee'] = setting_item_array('visa_booking_buyer_fees');
    ...
    {
        return "\Modules\Visa\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "3",
                "data" => Attributes::getAllAttributesForApi("visa")
            ]
        ];
    }
